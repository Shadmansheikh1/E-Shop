...
const Navbar = () => {
  ...
  <div className="container-fluid">
    ...
    <form className="d-flex m-auto" role="search">
      ...
      <button onClick={handleLogout} className='btn btn-danger'id='logout'>Logout</button>
      <button onClick={handleSignup} className='btn btn-success ms-2'>Signup</button>
      <ul className="navbar-nav ms-auto mb-2 mb-lg-0">
        <li className="nav-item">
          <Link className="nav-link active" aria-current="page" to="/">Home</Link>
        </li>
        <li className="nav-item">
          <Link className="nav-link" to="/cart"><PiShoppingCartLight size={'30px'}/>{ctx.cartItem.length} </Link>
        </li>
